...
function buildFeaturedCard(v) {
  ...
  const thumb = thumbUrl(v.id);
  return `
    <div class="featured-card" style="margin-bottom:24px;">
      <div class="featured-thumb" style="background-image:url('${thumb}');background-size:cover;background-position:center;">
...
function buildVideoCard(v) {
  ...
  const seriesLabel = v.seriesLabel ? `<span class="series-label">${v.seriesLabel}</span>` : '';
  return `
